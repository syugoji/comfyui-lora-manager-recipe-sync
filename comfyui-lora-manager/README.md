> ## ⚠️ これは非公式フォークです / This is an unofficial fork
>
> **本家ではありません。** [willmiao/ComfyUI-Lora-Manager](https://github.com/willmiao/ComfyUI-Lora-Manager)
> の **v1.1.6 を基点に改変**したものです。upstream の 1.1.9 以降には追従していません。
> 本家へバグ報告を送る前に、まずこのフォークを外して再現するか確かめてください。
>
> - 変更点の一覧: **[CHANGES-FORK.md](CHANGES-FORK.md)**
> - ライセンス: upstream と同じ **GPL-3.0**（[LICENSE](LICENSE)）
> - 主な変更: レシピの生成元を「画像ファイル埋め込み優先」に変更／ワークフロー復元／
>   ローカルパス形式の LoRA 解決／実行リスト／内部API(`api/trpc`)の撤去
>
> *Not the original project. Forked from upstream **v1.1.6**, not tracking 1.1.9+.
> See [CHANGES-FORK.md](CHANGES-FORK.md) for the full list of modifications. GPL-3.0, same as upstream.*
...
- 🧩 **LoRA Recipes**
  ...
  - Recreate embedded ComfyUI workflows first, then use ComfyUI's native A1111 image-metadata conversion when available
  - Keep reconstructed txt2img samplers at full denoise when they start from an empty latent
  - Resolve recipe downloads to the exact CivitAI file variant by file ID and SHA-256
