...
async def test_open_backup_location_uses_settings_directory(tmp_path, monkeypatch):
    ...
    assert calls == [[opener, str(backup_dir)]]
...
async def test_open_wildcards_location_creates_and_opens_directory(tmp_path, monkeypatch):
    ...
    opener = "explorer" if os.name == "nt" else ("open" if sys.platform == "darwin" else "xdg-open")
    assert calls == [[opener, str(wildcards_dir)]]
...
@pytest.mark.asyncio
async def test_load_recipe_workflow_targets_requested_comfy_tab():
    send_calls: list[tuple[str, dict, str | None]] = []

    class RecordingPromptServer:
        class Instance:
            sockets = {"client-a": object(), "client-b": object()}

            def send_sync(self, event, payload, sid=None):
                send_calls.append((event, payload, sid))
    ...
    response = await handler.load_recipe_workflow(
        FakeRequest(
            json_data={
                "prompt": prompt,
                "title": "Recipe",
                "workflow_name": "Recipe_recipe-1_123",
                "source": "standard",
...
@pytest.mark.asyncio
async def test_load_recipe_workflow_uses_server_registered_focused_tab():
    send_calls: list[tuple[str, dict, str | None]] = []
    ...
    active_response = await handler.set_active_client(
        FakeRequest(json_data={"client_id": "client-b"})
    )
    ...
    response = await handler.load_recipe_workflow(
        FakeRequest(json_data={"prompt": prompt, "target_client_id": "stale-client"})
    )
    payload = json.loads(response.text)

    assert response.status == 200
    assert payload["success"] is True
    assert payload["target_client_id"] == "client-b"
    assert send_calls[0][0] == "lm_load_recipe_workflow"
    assert send_calls[0][2] == "client-b"


@pytest.mark.asyncio
async def test_set_active_client_rejects_disconnected_tab():
    class RecordingPromptServer:
        class Instance:
            sockets = {"client-a": object()}
    ...
    response = await handler.set_active_client(
        FakeRequest(json_data={"client_id": "disconnected"})
    )
...
@pytest.mark.asyncio
async def test_load_recipe_workflow_resolves_bare_model_names_to_subfolders():
    send_calls: list[tuple[str, dict, str | None]] = []
    ...
    prompt = {
        "1": {
            "inputs": {"ckpt_name": "picxReal_10.safetensors"},
            "class_type": "CheckpointLoaderSimple",
        },
        "2": {
            "inputs": {
            ...
            "_meta": {"lora_aliases": ["Styles\\MoXin", "MoXinV1"]},
        },
        "3": {
            "inputs": {
                "model": ["2", 0],
    ...
    def get_filename_list(folder_name):
        return {
            "checkpoints": ["SD 1.5\\photorealistic\\picxReal_10.safetensors"],
            "loras": ["SD 1.5\\style\\MoXinV1.safetensors"],
        }.get(folder_name, [])

    with patch(
        "folder_paths.get_filename_list",
        side_effect=get_filename_list,
        create=True,
    ):
        response = await handler.load_recipe_workflow(
            FakeRequest(
                json_data={
                    ...
                    "a1111_checkpoint": "picxReal_10.safetensors",
                }
            )
        )

    payload = json.loads(response.text)
    sent_prompt = send_calls[0][1]["prompt"]

    assert response.status == 200
    assert sent_prompt["1"]["inputs"]["ckpt_name"] == (
        "SD 1.5\\photorealistic\\picxReal_10.safetensors"
    )
    assert sent_prompt["2"]["inputs"]["lora_name"] == (
        "SD 1.5\\style\\MoXinV1.safetensors"
    )
    assert send_calls[0][1]["a1111_loras"] == [
        {
            "name": "SD 1.5\\style\\MoXinV1.safetensors",
            "strength_model": 0.7,
            "strength_clip": 0.6,
            "aliases": ["Styles\\MoXin", "MoXinV1"],
        }
    ]
    assert send_calls[0][1]["a1111_checkpoint"] == (
        "SD 1.5\\photorealistic\\picxReal_10.safetensors"
    )
    assert len(payload["resolved_models"]) == 2
    assert payload["unresolved_models"] == []
...
@pytest.mark.asyncio
async def test_load_recipe_workflow_prepare_only_returns_normalized_prompt_without_tab():
    send_calls: list[tuple[str, dict, str | None]] = []
    ...
    def get_filename_list(folder_name):
        return {
            ...
            "loras": ["Styles\\style.safetensors"],
        }.get(folder_name, [])

    with patch("folder_paths.get_filename_list", side_effect=get_filename_list, create=True):
        response = await handler.load_recipe_workflow(
            FakeRequest(
                json_data={
                    "prompt": prompt,
    ...
    assert response.status == 200
    assert payload["success"] is True
    assert payload["prompt"]["1"]["inputs"]["ckpt_name"] == "SDXL\\model.safetensors"
    assert payload["prompt"]["2"]["inputs"]["lora_name"] == "Styles\\style.safetensors"
    assert payload["unresolved_models"] == []
    assert payload["skipped_models"] == []
    assert send_calls == []
...
@pytest.mark.asyncio
async def test_load_recipe_workflow_rejects_unresolved_required_lora():
    send_calls: list[tuple[str, dict, str | None]] = []
    ...
    prompt = {
        ...
        "2": {
            "inputs": {
                "model": ["1", 0],
                "clip": ["1", 1],
                "lora_name": "required-missing.safetensors",
                "strength_model": 0.9,
                "strength_clip": 0.9,
            },
            "class_type": "LoraLoader",
    ...
    def get_filename_list(folder_name):
        return {"checkpoints": ["model.safetensors"], "loras": []}.get(folder_name, [])
    ...
    assert payload["unresolved_models"] == [
        {
            ...
            "widget_name": "lora_name",
            "folder_name": "loras",
            "requested": "required-missing.safetensors",
            "required": True,
            "requirement_id": "recipe:0",
        }
    ]
...
@pytest.mark.asyncio
async def test_load_recipe_workflow_resolves_compatible_vae_and_t5_families():
    send_calls: list[tuple[str, dict, str | None]] = []
    ...
    with patch(
        ...
        response = await handler.load_recipe_workflow(
            FakeRequest(json_data={"prompt": prompt})
        )

    payload = json.loads(response.text)
    sent_prompt = send_calls[0][1]["prompt"]
    assert response.status == 200
    assert sent_prompt["1"]["inputs"]["vae_name"] == "sdxl_vae.safetensors"
    assert sent_prompt["2"]["inputs"]["clip_name1"] == (
        "t5xxl_fp8_e4m3fn_scaled.safetensors"
    )
    assert len(payload["resolved_models"]) == 2
    assert all(
        item["reason"] == "compatible installed model family"
        for item in payload["resolved_models"]
    )


@pytest.mark.asyncio
async def test_load_recipe_workflow_rejects_unavailable_upscaler_instead_of_substituting():
    send_calls: list[tuple[str, dict, str | None]] = []
    ...
    prompt = {
        ...
        "2": {
            "inputs": {"model_name": "SwinIR_4x"},
            "class_type": "UpscaleModelLoader",
        },
        "3": {
            "inputs": {"ckpt_name": "anythingelseV4_v45"},
            "class_type": "CheckpointLoaderSimple",
        },
    }
    ...
    def get_filename_list(folder_name):
        return {
            "checkpoints": ["SD 1.5\\anime\\anythingelseV4_v45.ckpt"],
            "loras": ["SD 1.5\\anime\\animeoutlineV4_16.safetensors"],
            "upscale_models": ["RealESRGAN_x4.pth", "remacri_original.pth"],
        }.get(folder_name, [])
        ...
        response = await handler.load_recipe_workflow(
            FakeRequest(
                json_data={
                    ...
                    "a1111_parameters": (
                        "portrait\nNegative prompt: bad\n"
                        "Steps: 20, Hires upscaler: SwinIR_4x"
                    ),
                }
            )
        )
    ...
    assert response.status == 422
    assert send_calls == []
    assert payload["error"] == "Required workflow models are unavailable"
    assert payload["unresolved_models"] == [
        {
            "node_id": "2",
...
@pytest.mark.asyncio
async def test_load_recipe_workflow_rejects_ambiguous_target():
    class MultiTabPromptServer:
        class Instance:
            sockets = {"client-a": object(), "client-b": object()}

            def send_sync(self, event, payload, sid=None):
                raise AssertionError("workflow should not be sent")

        instance = Instance()

    handler = NodeRegistryHandler(
        node_registry=NodeRegistry(),
        prompt_server=MultiTabPromptServer,
        standalone_mode=False,
    )
    response = await handler.load_recipe_workflow(
        FakeRequest(
