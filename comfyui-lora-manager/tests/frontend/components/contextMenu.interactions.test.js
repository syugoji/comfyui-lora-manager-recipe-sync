...
const createRecipePromptDraftMock = vi.fn();
const releaseRecipePromptModelMock = vi.fn();
const adoptRecipeRevisionMock = vi.fn();
const startRecipeTrialsMock = vi.fn();
const recoverRecipeTrialsMock = vi.fn();
const cancelRecipeTrialsMock = vi.fn();
...
vi.mock('../../../static/js/api/recipeApi.js', () => ({
  adoptRecipeRevision: adoptRecipeRevisionMock,
  createRecipePromptDraft: createRecipePromptDraftMock,
  fetchRecipeDetails: fetchRecipeDetailsMock,
  releaseRecipePromptModel: releaseRecipePromptModelMock,
  updateRecipeMetadata: updateRecipeMetadataMock,
}));

vi.mock('../../../static/js/managers/RecipeTrialManager.js', () => ({
  recipeTrialManager: {
    start: startRecipeTrialsMock,
    recover: recoverRecipeTrialsMock,
    cancel: cancelRecipeTrialsMock,
  },
}));
...
describe('Interaction-level regression coverage', () => {
  beforeEach(() => {
    ...
    createRecipePromptDraftMock.mockResolvedValue(null);
    releaseRecipePromptModelMock.mockResolvedValue([]);
    adoptRecipeRevisionMock.mockResolvedValue(null);
    startRecipeTrialsMock.mockResolvedValue(null);
    recoverRecipeTrialsMock.mockResolvedValue(null);
    cancelRecipeTrialsMock.mockResolvedValue(null);
  ...
  it('shows the manifest replay weight instead of a stale catalog weight', async () => {
    document.body.innerHTML = `
      <div id="recipeModal" class="modal">
        <div id="recipeCheckpoint"></div>
        <div id="recipeResourceDivider"></div>
    ...
    const { RecipeModal } = await import('../../../static/js/components/RecipeModal.js');
    const recipeModal = new RecipeModal();
    recipeModal.syncResourcesSection({
      loras: [{
        modelName: 'Embedded LoRA',
        modelVersionName: 'v1',
      ...
      replay_manifest: {
        required_resources: [{
          requirement_id: 'recipe:0',
          kind: 'lora',
          required: true,
          expected: { strength_model: 0.3, strength_clip: 0.3 },
        }],
      },
    });

    expect(document.getElementById('recipeLorasList').textContent).toContain('Replay weight: 0.3');
    expect(document.getElementById('recipeLorasList').textContent).not.toContain('Weight: 1');
  });
  ...
  it('renders a local AI prompt draft without saving or interpreting returned HTML', async () => {
    document.body.innerHTML = `
      <button id="createAiPromptDraftBtn"></button>
      <button id="regenerateAiPromptDraftBtn"></button>
      <select id="aiPromptModelSelect">
        <option value="qwen35-q6">Qwen</option>
        <option value="qwythos-q5">Qwythos</option>
      </select>
      <button id="copyAiPromptDraftBtn"></button>
      <button id="startAiPromptTrialsBtn" disabled></button>
      <button id="refreshAiPromptTrialsBtn"></button>
      <button id="cancelAiPromptTrialsBtn" hidden></button>
      <section id="aiPromptDraftPanel" hidden>
        <span id="aiPromptDraftStatus"></span>
        <div id="aiPromptImageSource"></div>
        <div id="aiPromptDraftWarning" hidden></div>
        <div id="aiPromptProtectedFragments"></div>
        <p id="aiPromptDescription"></p>
        <textarea id="aiPromptDraftText"></textarea>
        <div id="aiPromptTrialStatus"></div>
        <div id="aiPromptTrialGrid" hidden></div>
      </section>
    `;
    const manifestHash = 'a'.repeat(64);
    createRecipePromptDraftMock.mockResolvedValue({
      proposed_prompt: '<lora:USNR:0.9>, (usnr), detailed portrait',
      description: '<img src=x onerror=alert(1)> detailed portrait',
      protected: {
        lora_tags: ['<lora:USNR:0.9>'],
        trigger_tokens: ['(usnr)'],
        embeddings: [],
      },
      image: {
        preview_used: true, input_width: 480, input_height: 701,
        warning: 'プレビュー画像を使用しました。',
      },
      lm_studio: {
        model_label: 'Qwen3.5 9B Q6_K',
        cache_hit: true,
      },
    });
    const { RecipeModal } = await import('../../../static/js/components/RecipeModal.js');
    ...
    recipeModal.currentRecipe = {
      id: 'recipe-ai', replay_manifest: { manifest_hash: manifestHash },
    };
    recipeModal.recipeId = 'recipe-ai';

    await recipeModal.requestPromptDraft();

    expect(createRecipePromptDraftMock).toHaveBeenCalledWith(
      'recipe-ai', {
        manifestHash,
        model: 'qwen35-q6',
    ...
    expect(document.getElementById('aiPromptDraftStatus').textContent).toContain('保存済み結果');
    expect(document.getElementById('aiPromptDraftPanel').hidden).toBe(false);
    expect(document.getElementById('aiPromptDraftText').value).toContain('(usnr)');
    expect(document.getElementById('aiPromptDraftWarning').textContent).toContain('プレビュー');
    expect(document.querySelector('#aiPromptDescription img')).toBeNull();
    expect(document.getElementById('aiPromptDescription').textContent).toContain('<img');
    expect(document.getElementById('startAiPromptTrialsBtn').disabled).toBe(false);
    expect(updateRecipeMetadataMock).not.toHaveBeenCalled();

    document.getElementById('aiPromptModelSelect').value = 'qwythos-q5';
    document.getElementById('aiPromptModelSelect').dispatchEvent(new Event('change'));
    await recipeModal.requestPromptDraft({ forceRegenerate: true });
    expect(createRecipePromptDraftMock).toHaveBeenLastCalledWith(
      'recipe-ai', {
        manifestHash,
        model: 'qwythos-q5',
  ...
  it('renders four sequential trial candidates without writing the original recipe', async () => {
    document.body.innerHTML = `
      <section id="recipeRevisionBanner" hidden>
        <div id="recipeRevisionMeta"></div>
        <p id="recipeRevisionPrompt"></p>
      </section>
      <section id="aiPromptDraftPanel">
        <button id="startAiPromptTrialsBtn"></button>
        <button id="refreshAiPromptTrialsBtn"></button>
        <button id="cancelAiPromptTrialsBtn" hidden></button>
        <div id="aiPromptTrialStatus"></div>
        <div id="aiPromptTrialGrid" hidden></div>
    ...
    `;
    const recipe = {
      id: 'recipe-ai', replay_manifest: { manifest_hash: 'a'.repeat(64) },
      source_etag: 'e'.repeat(64),
      gen_params: { seed: 42 },
    };
    const draft = {
      schema: 'lora-manager.prompt-draft', version: 1,
      draft_hash: 'd'.repeat(64), manifest_hash: 'a'.repeat(64),
      prompt_source: 'lm_studio',
      proposed_prompt: '<lora:USNR:0.9>, (usnr), detailed portrait',
      negative_prompt: 'bad hands',
    };
    const job = {
      recipe_id: 'recipe-ai', status: 'completed', active_index: null,
      source_etag: recipe.source_etag, manifest_hash: draft.manifest_hash,
      draft_hash: draft.draft_hash, draft_snapshot: draft,
      candidates: [0, 1, 2, 3].map(index => ({
        index, seed: 42 + index, seed_origin: index === 0 ? 'original' : 'random',
        candidate_id: `recipe-ai:1000:${index}`,
        prompt_id: `00000000-0000-4000-8000-00000000000${index}`,
        status: 'succeeded', error: null,
        images: [{
          url: `/view?filename=candidate-${index}.png&type=output`,
          output_node_id: '9', image_index: 0,
        }],
      })),
    };
    adoptRecipeRevisionMock.mockResolvedValue({
      success: true,
      revision_id: 'revision-1',
      revision_summary: {
        active: true, stale: false, prompt_source: 'lm_studio',
        seed: 42, model: 'Qwythos Q5',
      },
      original_recipe_changed: false,
    });
    fetchRecipeDetailsMock.mockResolvedValue(recipe);
    startRecipeTrialsMock.mockImplementation(async ({ onUpdate }) => {
      onUpdate(job);
      return job;
    });
    const { RecipeModal } = await import('../../../static/js/components/RecipeModal.js');
    const recipeModal = new RecipeModal();
    recipeModal.currentRecipe = recipe;
    recipeModal.recipeId = 'recipe-ai';
    recipeModal.currentPromptDraft = draft;

    await recipeModal.startPromptTrials();

    expect(releaseRecipePromptModelMock).toHaveBeenCalledWith('recipe-ai');
    expect(startRecipeTrialsMock).toHaveBeenCalledWith(expect.objectContaining({ recipe, draft }));
    expect(document.querySelectorAll('.ai-prompt-trial-card')).toHaveLength(4);
    expect(document.querySelectorAll('.ai-prompt-trial-image')).toHaveLength(4);
    expect(document.querySelectorAll('.ai-prompt-trial-adopt')).toHaveLength(4);
    expect(document.getElementById('aiPromptTrialStatus').textContent).toContain('すべて完了');
    expect(updateRecipeMetadataMock).not.toHaveBeenCalled();

    document.querySelector('.ai-prompt-trial-adopt').click();
    await flushAsyncTasks();

    expect(adoptRecipeRevisionMock).toHaveBeenCalledWith('recipe-ai', {
      sourceEtag: recipe.source_etag,
      manifestHash: draft.manifest_hash,
      draft,
      candidate: {
        candidate_id: 'recipe-ai:1000:0',
        prompt_id: '00000000-0000-4000-8000-000000000000',
        output_node_id: '9', image_index: 0, seed: 42,
      },
    });
    expect(document.getElementById('recipeRevisionBanner').hidden).toBe(false);
    expect(document.getElementById('recipeRevisionPrompt').textContent).toContain('detailed portrait');
    expect(recipeModal.currentRecipe.gen_params).toEqual({ seed: 42 });
    expect(updateRecipeMetadataMock).not.toHaveBeenCalled();
  });
});
