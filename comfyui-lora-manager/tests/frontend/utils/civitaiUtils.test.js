...
describe('civitaiUtils', () => {
    ...
    describe('Civitai page URL helpers', () => {
        ...
        it('rejects zero IDs and falls back to a model-name search', () => {
            expect(buildCivitaiModelUrl(0, 0, 'civitai.red')).toBe(null);
            expect(buildCivitaiUrl({
                modelId: 0,
                versionId: '0',
                modelName: 'legacy lora',
                host: 'civitai.red',
            })).toBe('https://civitai.red/models?query=legacy%20lora');
        });

        it('builds CivArchive links only from positive model IDs', () => {
            expect(buildCivArchiveModelUrl(8214, 13304)).toBe(
                'https://civarchive.com/models/8214?modelVersionId=13304'
            );
            expect(buildCivArchiveModelUrl(0, 13304)).toBe(null);
        });
