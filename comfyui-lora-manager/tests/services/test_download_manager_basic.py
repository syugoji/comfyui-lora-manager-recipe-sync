...
def test_pt_zip_serialization_is_not_treated_as_download_archive(tmp_path):
    """PyTorch's ZIP-backed .pt files must remain direct model downloads."""
    manager = DownloadManager()
    serialized_model = tmp_path / "firekeeper.pt"
    download_archive = tmp_path / "bundle.zip"

    for path in (serialized_model, download_archive):
        with zipfile.ZipFile(path, "w") as archive:
            archive.writestr("data.pkl", b"pytorch serialization")

    assert manager._is_model_archive(str(serialized_model)) is False
    assert manager._is_model_archive(str(download_archive)) is True
    assert ".pt" in manager._get_supported_extensions_for_type("lora")
...
@pytest.mark.asyncio
async def test_download_is_idempotent_when_version_exists(
    monkeypatch, scanners, metadata_provider
):
    """Test that an installed version is reported as a successful skip."""
    ...
    assert result["model_version_id"] == 101
    assert "download_id" in result
...
async def test_download_is_idempotent_when_hash_exists_without_version_metadata(
    ...
    metadata_provider.get_model_version = AsyncMock(
        return_value={
            "id": 13304,
            "model": {"type": "LoRA", "tags": ["character"]},
            "baseModel": "SD 1.4",
            "files": [
                {
                    "type": "Model",
    ...
    assert result["already_exists"] is True
    assert result["reason"] == "already_in_library"
    assert result["model_version_id"] == 13304
    assert execute_mock.await_count == 0


@pytest.mark.asyncio
async def test_download_selects_exact_non_primary_recipe_file(
    monkeypatch, scanners, tmp_path
):
    """An exact recipe file must not be skipped just because its version exists."""
    scanners.lora.exists = True
                ...
                "files": [
                    {
                        ...
                        "type": "Model",
                        "primary": True,
                        "downloadUrl": "https://example.invalid/pruned.safetensors",
                        "name": "pruned.safetensors",
                        "hashes": {"SHA256": "C060E0173D"},
                    },
                    {
                        "id": 181973,
                        "type": "Model",
                        "primary": False,
                        "downloadUrl": "https://example.invalid/full.safetensors",
                        "name": "full.safetensors",
                        "hashes": {"SHA256": "7EB8672531ABCDEF"},
                        "metadata": {
                            "format": "SafeTensor",
                            "size": "full",
                            "fp": "fp32",
    ...
    assert result["success"] is True
    assert execute_mock.await_args.kwargs["download_urls"] == [
        "https://example.invalid/full.safetensors"
    ]
    assert execute_mock.await_args.kwargs["metadata"].file_path.endswith(
        "full.safetensors"
    )
...
@pytest.mark.asyncio
async def test_download_selects_exact_file_from_unique_short_sha_prefix(
    monkeypatch, scanners, tmp_path
):
    provider = SimpleNamespace(
        get_model_version=AsyncMock(
            return_value={
                "id": 1550840,
                "model": {"type": "TextualInversion", "tags": []},
                ...
                "files": [
                    {
                        ...
                        "type": "Model",
                        "primary": True,
                        "downloadUrl": "https://example.invalid/lazyhand.safetensors",
                        "name": "lazyhand.safetensors",
                        "hashes": {
                            "SHA256": "3CC8F76AAF5A5B38CF41CC4C0286F59778D7C76FDCC6E7725612277DEEF783F4"
                        },
                    }
                ],
            }
    ...
    manager = DownloadManager()
    result = await manager.download_from_civitai(
        model_version_id=1550840,
        save_dir=str(tmp_path),
        file_params={"sha256": "3cc8f76aaf5a"},
    )

    assert result["success"] is True
    assert execute_mock.await_args.kwargs["download_urls"] == [
        "https://example.invalid/lazyhand.safetensors"
    ]
