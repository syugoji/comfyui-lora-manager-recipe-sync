...
class TestModelHashIndexShortHashes:
    def test_unique_sha_prefix_resolves_to_local_model(self):
        index = ModelHashIndex()
        full_hash = "3cc8f76aaf5a5b38cf41cc4c0286f59778d7c76fdcc6e7725612277deef783f4"
        index.add_entry(full_hash, "/models/lazyhand.safetensors")

        assert index.has_hash("3cc8f76aaf5a") is True
        assert index.get_path("3cc8f76aaf5a") == "/models/lazyhand.safetensors"

    def test_ambiguous_sha_prefix_is_not_accepted(self):
        index = ModelHashIndex()
        index.add_entry("12345678aaaa", "/models/a.safetensors")
        index.add_entry("12345678bbbb", "/models/b.safetensors")

        assert index.has_hash("12345678") is False
        assert index.get_path("12345678") is None

    def test_too_short_sha_prefix_is_not_accepted(self):
        index = ModelHashIndex()
        index.add_entry("12345678aaaa", "/models/a.safetensors")

        assert index.has_hash("1234567") is False
