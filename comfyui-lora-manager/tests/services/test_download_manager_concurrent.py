...
@pytest.mark.asyncio
async def test_execute_download_rejects_same_size_hash_mismatch(monkeypatch, tmp_path):
    manager = DownloadManager()
    save_dir = tmp_path / "downloads"
    save_dir.mkdir()
    target_path = save_dir / "file.safetensors"
    ...
    class DummyMetadata:
        def __init__(self, path: Path):
            self.file_path = str(path)
            self.file_name = path.stem
            self.sha256 = hashlib.sha256(expected_payload).hexdigest()
            self.preview_url = None
            self.preview_nsfw_level = None
    ...
    class DummyDownloader:
        ...
        async def download_file(self, _url, path, **_kwargs):
            Path(path).write_bytes(corrupt_payload)
            return True, path
