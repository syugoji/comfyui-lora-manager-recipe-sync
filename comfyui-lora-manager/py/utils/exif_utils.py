...
class ExifUtils:
    ...
    @staticmethod
    def extract_image_metadata_fields(image_path: str) -> dict[str, Optional[str]]:
        """Return all structured generation fields without discarding workflow data."""
        try:
            if image_path:
                ext = os.path.splitext(image_path)[1].lower()
                if ext in ['.mp4', '.webm']:
            ...
            return ExifUtils._load_structured_metadata(image_path)
        except Exception as e:
            logger.error(f"Error extracting structured image metadata: {e}", exc_info=True)
            return {
                "parameters": None,
                "prompt": None,
                "workflow": None,
