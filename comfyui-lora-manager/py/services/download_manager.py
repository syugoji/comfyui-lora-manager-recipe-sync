...
class DownloadManager:
    ...
    @staticmethod
    async def _path_matches_sha256(file_path: str, expected_sha256: str) -> bool:
        """Verify a provider SHA against the file bytes instead of sidecar metadata."""
        ...
        actual = await calculate_sha256(file_path)
    ...
    @staticmethod
    def _already_in_library_result(model_type: str, model_version_id: int) -> Dict:
        """Return an idempotent success when the requested version is installed."""
        ...
        return {
            ...
            "message": f"Model version already exists in {model_type} library",
    ...
    async def download_from_civitai(
        ...
        Args:
            ...
            file_params: Optional dict with file selection params (fileId, sha256,
                type, format, size, fp, isPrimary)
    ...
    async def _execute_original_download(
        ...
        try:
            exact_file_requested = bool(
                file_params
                and (file_params.get("fileId") is not None or file_params.get("sha256"))
            )

            # Check if model version already exists in library
            if model_version_id is not None and not exact_file_requested:
                # Check both scanners
                ...
                if await lora_scanner.check_model_version_exists(model_version_id):
                    return self._already_in_library_result(
            ...
            if model_version_id is None and not exact_file_requested:
                ...
                if model_type == "lora":
                    ...
                    if await lora_scanner.check_model_version_exists(version_id):
                        return self._already_in_library_result("lora", version_id)
            ...
            if file_params and model_version_id:
                target_file_id = file_params.get("fileId")
                target_sha256 = str(file_params.get("sha256") or "").strip().lower()
                ...
                if target_file_id is not None:
                    file_info = next(
                        (f for f in files if str(f.get("id")) == str(target_file_id)),
                        None,
                    )
                if not file_info and target_sha256:
                    hash_matches = [
                        f
                        for f in files
                        if len(target_sha256) >= 8
                        and str((f.get("hashes") or {}).get("SHA256") or "")
                        .strip()
                        .lower()
                        .startswith(target_sha256)
                    ]
                    if len(hash_matches) == 1:
                        file_info = hash_matches[0]

                if exact_file_requested and not file_info:
                    return {
                        "success": False,
                        "error": "Requested recipe file was not found in model metadata",
                    }

                if not file_info and is_primary:
                    # Find primary file
                    file_info = next(
                        (
                            f
            ...
            # Older or externally scanned model files may have a complete SHA256
            # but no Civitai version ID in their sidecar metadata.  Compare the
            # provider's expected hash before downloading so retries remain
            # idempotent even when the version index cannot identify the file.
            hashes = file_info.get("hashes") or {}
            expected_sha256 = str(hashes.get("SHA256") or "").strip().lower()
            if expected_sha256:
                if model_type == "lora":
                    hash_scanner = await self._get_lora_scanner()
                elif model_type == "checkpoint":
                    hash_scanner = await self._get_checkpoint_scanner()
                else:
                    hash_scanner = await ServiceRegistry.get_embedding_scanner()

                get_path_by_hash = getattr(hash_scanner, "get_path_by_hash", None)
                cached_path = (
                    get_path_by_hash(expected_sha256)
                    if callable(get_path_by_hash)
                    else None
                )
                if cached_path:
                    if await self._path_matches_sha256(cached_path, expected_sha256):
                        resolved_version_id = version_info.get("id")
                        if resolved_version_id is None:
                            resolved_version_id = model_version_id
                        return self._already_in_library_result(
                            model_type, resolved_version_id
                        )
                    logger.warning(
                        "Cached model hash does not match file bytes; redownloading %s",
                        cached_path,
                    )
    ...
    async def _execute_download(
        ...
        try:
            ...
            for download_url in download_urls:
                ...
                if success:
                    expected_sha256 = str(
                        getattr(metadata, "sha256", "") or ""
                    ).strip().lower()
                    should_verify_hash = len(expected_sha256) == 64 and all(
                        char in "0123456789abcdef" for char in expected_sha256
                    )
                    actual_sha256 = (
                        await calculate_sha256(save_path)
                        if should_verify_hash
                        else expected_sha256
                    )
                    if not should_verify_hash or (
                        actual_sha256 and actual_sha256.lower() == expected_sha256
                    ):
                        break
                    ...
                    logger.error("%s: %s", last_error, save_path)
                    if os.path.exists(save_path):
                        try:
                            os.remove(save_path)
                        except OSError as exc:
                            logger.warning(
                                "Failed to remove hash-mismatched download %s: %s",
                                save_path,
                                exc,
                            )
                    continue
            ...
            # PyTorch .pt/.pth files commonly use ZIP serialization internally.
            # They are model files, not distribution archives, so extraction
            # must be gated by the filename extension as well as ZIP content.
            if self._is_model_archive(save_path):
    ...
    @staticmethod
    def _is_model_archive(file_path: str) -> bool:
        """Return true only for explicit ZIP downloads, not ZIP-backed tensors."""
        return (
            os.path.splitext(file_path)[1].lower() == ".zip"
