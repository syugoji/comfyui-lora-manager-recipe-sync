...
class ModelScanner:
    ...
    def adjust_cached_entry(self, entry: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Hook for subclasses: adjust entries loaded from the persisted cache."""
        file_path = entry.get('file_path') or entry.get('path')
        if (
            isinstance(file_path, str)
            and file_path.lower().endswith('.safetensors')
            and os.path.isfile(file_path)
            and not is_valid_safetensors_file(file_path)
        ):
            logger.error("Ignoring structurally invalid safetensors cache entry: %s", file_path)
            return None
        return entry
    ...
    async def _process_model_file(
        ...
        *,
        ...
        if file_path.lower().endswith('.safetensors'):
            valid_container = await asyncio.to_thread(is_valid_safetensors_file, file_path)
            if not valid_container:
                logger.error("Skipping structurally invalid safetensors model: %s", file_path)
                return None
