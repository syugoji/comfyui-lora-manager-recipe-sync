...
class ModelHashIndex:
    ...
    def has_hash(self, hash_value: str) -> bool:
        """Check exact SHA256 or an unambiguous shortened SHA prefix."""
        return self.get_path(hash_value) is not None

    def get_path(self, hash_value: str) -> Optional[str]:
        """Get a path for an exact SHA256 or unique 8+ character prefix."""
        normalized = hash_value.lower()
        path = self._hash_to_path.get(normalized)
        if path is not None:
            return path
        if 8 <= len(normalized) < 64:
            matches = [
                candidate_path
                for sha256, candidate_path in self._hash_to_path.items()
                if sha256.startswith(normalized)
            ]
            if len(matches) == 1:
                return matches[0]
        return None
