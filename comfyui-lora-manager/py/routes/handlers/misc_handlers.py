...
def _workflow_model_folder(class_type: str, widget_name: str) -> str | None:
    if class_type == "UpscaleModelLoader" and widget_name == "model_name":
        return "upscale_models"
    return _WORKFLOW_MODEL_INPUT_FOLDERS.get(widget_name)


def _normalize_workflow_model_names(
    prompt: dict[str, Any],
) -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    """Resolve bare model basenames to ComfyUI's relative dropdown values."""

    getter = getattr(folder_paths, "get_filename_list", None)
    if not callable(getter):
        return [], []

    choices_by_folder: dict[str, list[str]] = {}
    ...
    def compatible_matches(
        folder_name: str, raw_value: str, choices: list[str]
    ) -> list[str]:
        raw_stem = re.sub(
            r"[^a-z0-9]+",
            "",
            os.path.splitext(normalized(raw_value).rsplit("/", 1)[-1])[0],
        )
        if folder_name == "vae":
            sdxl_aliases = {
                "sdxlvae",
                "sdxlvaefixed",
                "fixfp16errorssdxllowermemoryusev10",
            }
            if raw_stem in sdxl_aliases:
                return [
                    choice
                    ...
                    if re.sub(
                        r"[^a-z0-9]+",
                        "",
                        os.path.splitext(normalized(choice).rsplit("/", 1)[-1])[0],
                    )
                    == "sdxlvae"
                ]
        if folder_name == "text_encoders" and raw_stem.startswith("t5xxl"):
            return [
                choice
                for choice in choices
                if re.sub(
    ...
    for node_id, node in prompt.items():
        ...
        widget_names = list(_WORKFLOW_MODEL_INPUT_FOLDERS)
        if class_type == "UpscaleModelLoader":
            widget_names.append("model_name")

        for widget_name in widget_names:
            folder_name = _workflow_model_folder(class_type, widget_name)
            if not folder_name:
                continue
            raw_value = inputs.get(widget_name)
            if not isinstance(raw_value, str) or not raw_value.strip():
                continue

            if folder_name not in choices_by_folder:
                try:
                    choices = getter(folder_name)
                except Exception:
                    choices = []
                choices_by_folder[folder_name] = [
                    str(choice) for choice in choices or [] if isinstance(choice, str)
                ]

            choices = choices_by_folder[folder_name]
            raw_key = normalized(raw_value)
            exact_matches = [
                choice for choice in choices if normalized(choice) == raw_key
            ]
            matches = exact_matches
            if not matches:
                raw_basename = raw_key.rsplit("/", 1)[-1]
                matches = [
                    choice
                    for choice in choices
                    if normalized(choice).rsplit("/", 1)[-1] == raw_basename
                ]
            if not matches:
                raw_stem = os.path.splitext(raw_key.rsplit("/", 1)[-1])[0]
                matches = [
                    choice
                    for choice in choices
                    if os.path.splitext(
                        normalized(choice).rsplit("/", 1)[-1]
                    )[0]
                    == raw_stem
                ]
            compatible = False
            if not matches:
                matches = compatible_matches(folder_name, raw_value, choices)
            ...
            if len(matches) == 1:
                matched = matches[0]
                inputs[widget_name] = matched
                if matched != raw_value:
                    resolution = {**item, "resolved": matched}
                    if compatible:
                        resolution["reason"] = "compatible installed model family"
                    resolved.append(resolution)
            elif not matches:
                unresolved.append(item)
            else:
                unresolved.append({**item, "reason": "ambiguous basename"})

    return resolved, unresolved


def _bypass_unresolved_loras(
    prompt: dict[str, Any],
    unresolved: list[dict[str, str]],
    required_model_inputs: dict[tuple[str, str], str] | None = None,
) -> tuple[list[dict[str, str]], list[dict[str, str]]]:
    """Remove unavailable LoRA loaders without leaving broken graph links."""
    ...
    for item in unresolved:
        ...
        class_type = re.sub(
            r"[^a-z0-9]+", "", str(node.get("class_type") if isinstance(node, dict) else "").casefold()
        )
        inputs = node.get("inputs") if isinstance(node, dict) else None
        is_lora_loader = class_type.startswith("loraloader") or class_type.startswith(
            "loadlora"
        )
        if (
            item.get("folder_name") != "loras"
            or not is_lora_loader
            or not isinstance(inputs, dict)
        ):
            remaining.append(item)
            continue

        requirement = node.get("_meta", {}).get("replay_requirement")
        required_id = (required_model_inputs or {}).get((node_id, "lora_name"))
        if (isinstance(requirement, dict) and requirement.get("required") is True) or required_id:
            # Evidence-backed resources are an execution requirement.  Keep the
            # node intact so the caller receives a 422 instead of silently
            # loading a graph with the LoRA removed.
            remaining.append(
                {
                    **item,
                    "required": True,
                    "requirement_id": required_id or requirement.get("id"),
                }
            )
            continue
        ...
        for consumer in prompt.values():
            ...
            for key, value in list(consumer_inputs.items()):
                if not isinstance(value, list) or len(value) < 2 or str(value[0]) != node_id:
                    continue
                output_index = str(value[1])
                if output_index == "0" and isinstance(model_input, list):
                    consumer_inputs[key] = list(model_input)
                elif output_index == "1" and isinstance(clip_input, list):
                    consumer_inputs[key] = list(clip_input)
        prompt.pop(node_id, None)
        skipped.append(item)
    return skipped, remaining


def _normalize_a1111_upscaler(
    parameters: str,
    resolved_models: list[dict[str, str]],
) -> str:
    """Rewrite A1111's hires upscaler before the browser creates its node."""

    match = re.search(r"(?i)(Hires upscaler\s*:\s*)([^,\r\n]+)", parameters)
    if not match:
        return parameters
    ...
    requested_basename = requested_key.rsplit("/", 1)[-1]
    requested_stem = os.path.splitext(requested_basename)[0]
    existing = next(
        (
            item
            for item in resolved_models
            if item.get("folder_name") == "upscale_models"
            and (
                normalized(item.get("requested", "")) == requested_key
                or os.path.splitext(
                    normalized(item.get("requested", "")).rsplit("/", 1)[-1]
                )[0]
                == requested_stem
            )
        ),
    ...
    if resolution is None:
        ...
        try:
            choices = [
                str(choice)
                for choice in getter("upscale_models") or []
        ...
        matches = [choice for choice in choices if normalized(choice) == requested_key]
        if not matches:
            matches = [
                choice
                for choice in choices
                if normalized(choice).rsplit("/", 1)[-1] == requested_basename
            ]
        if not matches:
            matches = [
                choice
                for choice in choices
                if os.path.splitext(normalized(choice).rsplit("/", 1)[-1])[0]
                == requested_stem
            ]
        ...
        if len(matches) == 1:
            ...
            resolved = choices[0]
            reason = "fallback to installed upscale model"
        else:
            return parameters
        ...
        resolved_models.append(resolution)

    resolved = resolution.get("resolved")
    if not isinstance(resolved, str) or not resolved.strip():
        return parameters
    return f"{parameters[:match.start(2)]}{resolved.strip()}{parameters[match.end(2):]}"
...
class NodeRegistry:
    ...
    async def mark_active_client(self, sid: str) -> None:
        """Remember the ComfyUI tab that most recently reported user focus."""
        async with self._lock:
            self._active_client_id = sid

    async def get_active_client(self, active_sids: set[str]) -> str | None:
        """Return the focused tab only while its WebSocket is still connected."""
        async with self._lock:
            if self._active_client_id in active_sids:
...
class HealthCheckHandler:
    async def health_check(self, request: web.Request) -> web.Response:
        return web.json_response(
            {
                "status": "ok",
                "recipe_generation_source_policy": "embedded-first-v1",
            }
        )
...
class NodeRegistryHandler:
    ...
    async def set_active_client(self, request: web.Request) -> web.Response:
        """Record the ComfyUI workflow tab that most recently received focus."""
        try:
            data = await request.json()
            client_id = data.get("client_id")
            if not isinstance(client_id, str) or not client_id:
            ...
            sockets = self._prompt_server.instance.sockets
            if client_id not in sockets:
                return web.json_response(
                    {"success": False, "error": "ComfyUI client is not connected"},
                    status=409,
                )

            await self._node_registry.mark_active_client(client_id)
            return web.json_response({"success": True, "client_id": client_id})
        except Exception as exc:  # pragma: no cover - defensive logging
            logger.error("Failed to set active ComfyUI client: %s", exc, exc_info=True)
            return web.json_response({"success": False, "error": str(exc)}, status=500)
    ...
    async def load_recipe_workflow(self, request: web.Request) -> web.Response:
        """Load a reconstructed recipe prompt in a connected ComfyUI tab."""
        try:
            if self._standalone_mode:
                return web.json_response(
                    {
                        "success": False,
                        "error": "Standalone Mode Active",
                        "message": "Cannot load a workflow while LoRA Manager is running standalone.",
                    },
                    status=503,
                )
            ...
            prepare_only = data.get("prepare_only") is True
            a1111_parameters = data.get("a1111_parameters")
            requested_a1111_checkpoint = data.get("a1111_checkpoint")
            raw_required_model_inputs = data.get("required_model_inputs", [])

            if not isinstance(prompt, dict) or not prompt:
                return web.json_response(
                    {"success": False, "error": "prompt must be a non-empty object"},
                    status=400,
                )
            ...
            if not isinstance(raw_required_model_inputs, list) or len(raw_required_model_inputs) > 256:
                return web.json_response(
                    {"success": False, "error": "required_model_inputs must be a bounded array"},
                    status=400,
                )
            required_model_inputs: dict[tuple[str, str], str] = {}
            for entry in raw_required_model_inputs:
                if not isinstance(entry, dict):
                    return web.json_response(
                        {"success": False, "error": "required_model_inputs contains an invalid entry"},
                        status=400,
                    )
                node_id = str(entry.get("node_id") or "")
                widget_name = str(entry.get("widget_name") or "")
                requirement_id = str(entry.get("requirement_id") or "")
                node = prompt.get(node_id)
                node_type = re.sub(
                    r"[^a-z0-9]+",
                    "",
                    str(node.get("class_type") if isinstance(node, dict) else "").casefold(),
                )
                if (
                    not node_id
                    or widget_name != "lora_name"
                    or not requirement_id
                    or not (node_type.startswith("loraloader") or node_type.startswith("loadlora"))
                ):
                    return web.json_response(
                        {"success": False, "error": "required_model_inputs does not identify a LoRA loader"},
                        status=400,
                    )
                required_model_inputs[(node_id, widget_name)] = requirement_id

            if a1111_parameters is not None and not isinstance(a1111_parameters, str):
                return web.json_response(
                    {"success": False, "error": "a1111_parameters must be a string"},
                    status=400,
                )
            if isinstance(a1111_parameters, str) and len(a1111_parameters) > 250_000:
                return web.json_response(
                    {"success": False, "error": "a1111_parameters is too large"},
                    status=400,
                )
            if requested_a1111_checkpoint is not None and not isinstance(
                requested_a1111_checkpoint, str
            ):
                return web.json_response(
                    {"success": False, "error": "a1111_checkpoint must be a string"},
                    status=400,
                )

            resolved_models, unresolved_models = _normalize_workflow_model_names(
                prompt
            )
            skipped_models, unresolved_models = _bypass_unresolved_loras(
                prompt, unresolved_models, required_model_inputs
            )
            if unresolved_models:
                missing_names = list(
                    dict.fromkeys(
                        str(item.get("requested") or "").strip()
                        for item in unresolved_models
                        if str(item.get("requested") or "").strip()
                    )
                )
                preview = "、".join(missing_names[:4])
                remainder = (
                    f" ほか{len(missing_names) - 4}件"
                ...
                return web.json_response(
                    {
                        "success": False,
                        "error": "Required workflow models are unavailable",
                        "message": (
                            f"再現に必要なモデルをComfyUIで解決できません: {preview}{remainder}。"
                            "モデルの保存先またはレシピのモデル情報を確認してください。"
                        ),
            ...
            # Preserve the normalized LoRA chain for A1111 imports. ComfyUI's
            # native importer handles prompt/hires settings but ignores legacy
            # AddNet fields, so the frontend injects these resolved loaders as
            # ordinary <lora:...> tags immediately before import.
            a1111_loras = []
            a1111_checkpoint = None
            unresolved_model_inputs = {
                (item.get("node_id"), item.get("widget_name"))
                for item in unresolved_models
            }
            if isinstance(a1111_parameters, str) and a1111_parameters.strip():
                a1111_parameters = _normalize_a1111_upscaler(
                    a1111_parameters,
                    resolved_models,
                )
                for node_id, node in prompt.items():
                    ...
                    if (
                        requested_a1111_checkpoint
                        and class_type in {"CheckpointLoaderSimple", "UNETLoader"}
                    ):
                        widget_name = (
                            "ckpt_name"
                        ...
                        if (str(node_id), widget_name) not in unresolved_model_inputs:
                            candidate = inputs.get(widget_name)
                            requested_basename = os.path.basename(
                                requested_a1111_checkpoint.replace("\\", "/")
                            ).casefold()
                            candidate_basename = os.path.basename(
                                str(candidate or "").replace("\\", "/")
                            ).casefold()
                            if (
                                isinstance(candidate, str)
                                and candidate.strip()
                                and candidate_basename == requested_basename
                            ):
                                a1111_checkpoint = candidate.strip()
                    if class_type != "LoraLoader":
                        continue
                    name = inputs.get("lora_name")
                    if not isinstance(name, str) or not name.strip():
                        continue
                    aliases = node.get("_meta", {}).get("lora_aliases", [])
                    ...
                    a1111_loras.append(entry)

            if prepare_only:
                prepared_payload = {
                    "success": True,
                    "prompt": prompt,
                    "source": source if isinstance(source, str) else "standard",
                    "resolved_models": resolved_models,
                    "unresolved_models": unresolved_models,
                    "skipped_models": skipped_models,
                }
                if isinstance(a1111_parameters, str) and a1111_parameters.strip():
                    prepared_payload["a1111_parameters"] = a1111_parameters.strip()
                    if a1111_loras:
                        prepared_payload["a1111_loras"] = a1111_loras
                    if a1111_checkpoint:
                        prepared_payload["a1111_checkpoint"] = a1111_checkpoint
                return web.json_response(prepared_payload)

            sockets = self._prompt_server.instance.sockets
            active_client_ids = list(sockets.keys())
            if not active_client_ids:
                return web.json_response(
                    {
                        "success": False,
                        "error": "No ComfyUI tab connected",
                        "message": "Open ComfyUI in a browser tab and try again.",
                    },
                    status=409,
                )

            if not isinstance(target_client_id, str) or target_client_id not in sockets:
                focused_client_id = await self._node_registry.get_active_client(
                    set(active_client_ids)
                )
                if focused_client_id:
                    target_client_id = focused_client_id
                elif len(active_client_ids) == 1:
                    target_client_id = active_client_ids[0]
                else:
                    return web.json_response(
                        {
                            "success": False,
                            "error": "ComfyUI target tab is ambiguous",
                            "message": "Focus the ComfyUI tab once, return to LoRA Manager, and try again.",
                            "tab_count": len(active_client_ids),
                        },
                        status=409,
                    )
            ...
            payload = {
                ...
                "workflow_name": (
                    ...
                    else f"Recipe_{int(time.time() * 1000)}"
                ),
                "source": source if isinstance(source, str) else "standard",
            }
            if isinstance(a1111_parameters, str) and a1111_parameters.strip():
                payload["a1111_parameters"] = a1111_parameters.strip()
                if a1111_loras:
                    payload["a1111_loras"] = a1111_loras
                if a1111_checkpoint:
                    payload["a1111_checkpoint"] = a1111_checkpoint
            self._prompt_server.instance.send_sync(
                "lm_load_recipe_workflow",
                payload,
                sid=target_client_id,
            )
            return web.json_response(
                {
                    "success": True,
            ...
            logger.error("Failed to load recipe workflow: %s", exc, exc_info=True)
            return web.json_response({"success": False, "error": str(exc)}, status=500)
...
class MiscHandlerSet:
    ...
    def to_route_mapping(
        ...
        return {
            ...
            "set_active_client": self.node_registry.set_active_client,
            "update_node_widget": self.node_registry.update_node_widget,
            "load_recipe_workflow": self.node_registry.load_recipe_workflow,
