...
# Old A1111/AddNet metadata stores only the 12-character tensor hash.  Civitai
# and CivArchive now require the complete SHA-256 for by-hash lookups.  Keep
# only aliases whose full files were independently downloaded and verified.
VERIFIED_LEGACY_RESOURCE_HASHES = {
    "494301de3d6e": "f79768ec7b9e4f615458e0ea645424af183ffc0ebf020caab994eebe4dc84f7d",
    "acd58eb24484": "acd58eb244848f0ffddc647435ddd983eec5ceeb10c70baa6ca333a089715b69",
}
VERIFIED_LEGACY_RESOURCE_VERSIONS = {
    "494301de3d6e": 20143,
    "acd58eb24484": 13304,
}
...
class RecipeMetadataParser(ABC):
    ...
    @staticmethod
    def resolve_resource_hash_for_lookup(hash_value: Any) -> str:
        """Expand a verified legacy short hash without changing unknown hashes."""
        normalized = str(hash_value or "").strip().lower()
        return VERIFIED_LEGACY_RESOURCE_HASHES.get(normalized, normalized)

    @classmethod
    async def get_lora_metadata_by_hash(cls, metadata_provider, hash_value: Any):
        """Resolve a LoRA hash, with a verified version fallback for legacy AddNet."""
        normalized = str(hash_value or "").strip().lower()
        result = await metadata_provider.get_model_by_hash(
            cls.resolve_resource_hash_for_lookup(normalized)
        )
        data = result[0] if isinstance(result, tuple) else result
        if data:
            return result

        version_id = VERIFIED_LEGACY_RESOURCE_VERSIONS.get(normalized)
        version_getter = getattr(metadata_provider, "get_model_version_info", None)
        if version_id and callable(version_getter):
            return await version_getter(str(version_id))
        return result
    ...
    async def populate_lora_from_civitai(lora_entry: Dict[str, Any], civitai_info_tuple: Tuple[Dict[str, Any], Optional[str]], 
        ...
        try:
            ...
            if civitai_info.get('source'):
                lora_entry['metadataSource'] = civitai_info['source']
    ...
    @staticmethod
    async def populate_embedding_from_civitai(
        embedding: Dict[str, Any],
        civitai_info: Dict[str, Any],
        recipe_scanner=None,
        hash_value: Optional[str] = None,
    ) -> Optional[Dict[str, Any]]:
        """Populate a recipe embedding from a Civitai TextualInversion version."""
        try:
            civitai_data, error_msg = (
                (civitai_info, None)
                if not isinstance(civitai_info, tuple)
            ...
            model_type = str(
                (civitai_data.get("model") or {}).get("type") or ""
            ).lower().replace("_", "").replace(" ", "")
            if model_type not in {"textualinversion", "embedding"}:
                return None

            embedding["type"] = "embedding"
            embedding["isDeleted"] = False
            embedding["unresolved"] = False
            embedding["id"] = civitai_data.get("id", embedding.get("id", 0))
            embedding["modelId"] = civitai_data.get(
                "modelId", embedding.get("modelId", 0)
            )
            embedding["version"] = civitai_data.get("name", "")
            embedding["baseModel"] = civitai_data.get("baseModel", "")
            embedding["downloadUrl"] = civitai_data.get("downloadUrl", "")
            model_data = civitai_data.get("model") or {}
            if model_data.get("name"):
                embedding["name"] = model_data["name"]

            images = civitai_data.get("images") or []
            if images and images[0].get("url"):
                image_url = images[0]["url"]
                rewritten_image_url, _ = rewrite_preview_url(
                    image_url, media_type="image"
                )
                embedding["thumbnailUrl"] = rewritten_image_url or image_url

            files = civitai_data.get("files") or []
            model_file = next(
                (
                    file
                    for file in files
                    if file.get("type") == "Model" and file.get("primary") is True
                ),
                None,
            ) or next((file for file in files if file.get("type") == "Model"), None)
            if model_file:
                embedding["size"] = model_file.get("sizeKB", 0) * 1024
                sha256 = (model_file.get("hashes") or {}).get("SHA256") or hash_value
                if sha256:
                    embedding["hash"] = str(sha256).lower()
                file_name = model_file.get("name", "")
                if file_name:
                    embedding["file_name"] = os.path.splitext(file_name)[0]

            embedding_hash = str(embedding.get("hash") or "").lower()
            embedding_scanner = getattr(recipe_scanner, "_embedding_scanner", None)
            if embedding_scanner and embedding_hash:
                if embedding_scanner.has_hash(embedding_hash):
                    embedding["existsLocally"] = True
                    embedding["localPath"] = embedding_scanner.get_path_by_hash(
                        embedding_hash
                    )
                else:
                    embedding["existsLocally"] = False
            return embedding
        except Exception as exc:
            logger.error("Error populating embedding from Civitai info: %s", exc)
            return embedding
