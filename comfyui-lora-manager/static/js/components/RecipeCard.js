...
import { analyzeRecipeReplayCapability, getRecipePromptStatus } from '../utils/recipeReplayCapability.js';

const replayDetailsCache = new Map();
const REPLAY_DETAILS_CACHE_MS = 30_000;
...
class RecipeCard {
    ...
    createCardElement() {
        ...
        const notes = String(this.recipe.notes || '').trim();
        const promptStatus = getRecipePromptStatus(this.recipe);
        const revisionSummary = this.recipe.revision_summary || {};
        const hasActiveLmRevision = revisionSummary.active === true
            && String(revisionSummary.prompt_source || '').toLowerCase() === 'lm_studio';
            ...
            <div class="card-preview ${shouldBlur ? 'blurred' : ''}">
                ...
                <div class="card-header">
                    ...
                    <div class="card-actions">
                        ${notes ? '<i class="fas fa-sticky-note recipe-note-indicator" title="メモあり"></i>' : ''}
                        ...
                        <i class="fas fa-magic replay-workflow-action" title="チェックポイント・LoRA・生成条件をComfyUIに再現"></i>
                ...
                <span class="recipe-replay-capability analyzing" title="再現可否を確認中">
                    <i class="fas fa-spinner fa-spin"></i><span>確認中</span>
                </span>
                ${promptStatus === 'missing' ? `
                <span class="recipe-prompt-status missing" title="元画像にプロンプトがありません。モデルや雰囲気の参照専用です。">
                    <i class="fas fa-eye"></i><span>参照専用</span>
                </span>` : ''}
                ${promptStatus === 'generated' ? `
                <span class="recipe-prompt-status generated${hasActiveLmRevision ? ' modified' : ''}" title="AIで補完したプロンプトです。元画像のプロンプトではありません。">
                    <i class="fas fa-robot"></i><span>${hasActiveLmRevision ? 'AI補完・改変済み' : 'AI補完'}</span>
                </span>` : ''}
                ` : ''}
                ${shouldBlur ? `
        ...
        if (hasActiveLmRevision) {
            const badge = card.querySelector('.recipe-prompt-status.generated');
            const details = ['元レシピ未変更'];
            if (revisionSummary.stale === true) details.push('再生成が必要');
            if (revisionSummary.seed !== undefined && revisionSummary.seed !== null) {
                details.push(`seed ${revisionSummary.seed}`);
            }
            if (revisionSummary.model) details.push(String(revisionSummary.model));
            if (revisionSummary.created_at) details.push(String(revisionSummary.created_at));
            if (badge) badge.title = details.join(' / ');
        }

        if (notes) {
            const tooltip = document.createElement('div');
            tooltip.className = 'recipe-notes-tooltip';
            tooltip.textContent = notes;
            card.querySelector('.card-preview')?.appendChild(tooltip);
        }

        this.attachEventListeners(card, isDuplicatesMode, shouldBlur);
        if (!isDuplicatesMode) this.updateReplayCapability(card);
    ...
    async updateReplayCapability(card) {
        const badge = card.querySelector('.recipe-replay-capability');
        if (!badge) return;
        try {
            const recipeId = this.recipe.id || this.recipe.recipe_id;
            let recipe = this.recipe;
            if (recipeId) {
                const cached = replayDetailsCache.get(recipeId);
                if (!cached || Date.now() - cached.createdAt > REPLAY_DETAILS_CACHE_MS) {
                    const promise = fetchRecipeDetails(recipeId).catch(error => {
                        replayDetailsCache.delete(recipeId);
                        throw error;
                    });
                    replayDetailsCache.set(recipeId, { promise, createdAt: Date.now() });
                }
                recipe = await replayDetailsCache.get(recipeId).promise;
            }
            const capability = await analyzeRecipeReplayCapability(recipe);
            badge.className = `recipe-replay-capability ${capability.level}`;
            badge.title = capability.title;
            badge.innerHTML = `<i class="${capability.iconClass}"></i><span>${capability.label}</span>`;
            card.dataset.replayCapability = capability.level;
        } catch (error) {
            badge.className = 'recipe-replay-capability unavailable';
            badge.title = `再現可否の確認に失敗しました: ${error.message}`;
            badge.innerHTML = '<i class="fas fa-exclamation-circle"></i><span>判定失敗</span>';
            card.dataset.replayCapability = 'unavailable';
        }
    }
    ...
    attachEventListeners(card, isDuplicatesMode, shouldBlur) {
        ...
        if (!isDuplicatesMode) {
            ...
            card.querySelector('.replay-workflow-action')?.addEventListener('click', async (e) => {
                e.stopPropagation();
                const action = e.currentTarget;
                action.classList.add('fa-spin');
                try {
                    const { recipeWorkflowReplayManager } = await import('../managers/RecipeWorkflowReplayManager.js');
                    await recipeWorkflowReplayManager.replay(this.recipe);
                } catch (error) {
                    ...
                    action.classList.remove('fa-spin');
                }
            });
