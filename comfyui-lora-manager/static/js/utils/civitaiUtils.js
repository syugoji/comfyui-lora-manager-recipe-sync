...
export function buildCivArchiveModelUrl(modelId, versionId = null) {
    const normalizeId = value => {
        const text = value == null ? '' : String(value).trim();
        return /^\d+$/.test(text) && Number(text) > 0 ? text : '';
    };
    ...
    const query = normalizedVersionId
        ? `?modelVersionId=${encodeURIComponent(normalizedVersionId)}`
        : '';
    return `https://civarchive.com/models/${encodeURIComponent(normalizedModelId)}${query}`;
}
